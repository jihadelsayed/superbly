...
class DeleteSenderMessage(View):
    def get(self, request, user_message):
        ...
        template_name = 'delete_sender_message.html'
    ...
    def post(self, request, user_message):
        ...
        sender_only = message_only[1]

        try:
            userObj = Profile.objects.filter(username=sender_only.strip())
            friendObj = Friends.objects.filter(friend_id=userName, user_id=userObj[0].id)
            messageObj = Messages.objects.filter(user_id=userObj[0].id, friend_id=friendObj[0].id ,message=message_only[0].strip())
